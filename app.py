...
# ---------------------------------------------------------
# 3. CHIẾN LƯỢC PHÂN TÍCH (ĐÃ CẬP NHẬT FULL HEIKIN ASHI + BB)
# ---------------------------------------------------------
def run_strategy_full(df):
    if len(df) < 200: return df
    df = df.copy()
    
    # --- 1. TÍNH TOÁN HEIKIN ASHI (QUAN TRỌNG ĐỂ KHẮC PHỤC LỖI) ---
    ...
    # --- TÍNH TOÁN VÙNG MÂY XÁM (BOLLINGER BANDS) ---
...
# ---------------------------------------------------------
# 6. GIAO DIỆN CHÍNH (ĐÃ FIX LỖI INDENTATION VÀ NAME ERROR)
# ---------------------------------------------------------
...
if not st.session_state.logged_in:
    ...
    if symbol:
        ...
        if not d["error"]:
            ...
            with col_chart:
                # -------------------------------------------------------------
                # *** PLOTLY CHART: HEIKIN ASHI STYLE (ĐÃ FIX LỖI) ***
                # -------------------------------------------------------------
                fig = make_subplots(
                ...
                # 2. VẼ NẾN HEIKIN ASHI (Quan trọng)
                ...
                fig.update_yaxes(showgrid=True, gridcolor='rgba(255,255,255,0.1)', side="right")
